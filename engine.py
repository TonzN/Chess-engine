...
def pre_compute(board, depth=0):
    ...
    for square, piece in board.piece_map().items():
        if piece is not None:
            ...
            key ^= game_vars["zobrist_keys"][piece.piece_type][color][square]
        if piece.color == True:
...
def pawn_structure(board, team ,depth):
    pass
...
def mobility(board, team, depth):
    ...
    if game_vars["in_opening"] == False:
        mobility_score += 0.02 * len(legal_moves[4])
...
def negamax(board, depth, alpha, beta):
    ...
    if depth == game_vars["start_depth"]:
        key = zobrist_key(board)
        if key in transposition_table:
            return transposition_table[key]
        
        eval_score = eval(board, depth)
        transposition_table[key] = [eval_score, None, depth, key]
        return [eval_score, None, depth, key]
        
    best_move = None
   
    key = pre_compute(board, depth)
    if key in transposition_table:
        return transposition_table[key]
    else:
        transposition_table[key] = [eval(board, depth), None, depth, key]
        
    legal_moves = legal_moves_white[depth]
    if board.turn == False:
        legal_moves = legal_moves_black[depth]
        
    for move in legal_moves:
        board.push(move)
        result = negamax(board, depth + 1, -beta, -alpha)
        score = -result[0]
        board.pop()
    ...
    return [alpha, best_move, depth, key]
...
def run(last_move, board):
  ...
  #  time.sleep(0.25)
    ...
    if game_vars["in_opening_book"]:
        ...
        search_data = negamax(board, 0, float('-inf'), float('inf'))
        time_new = time.perf_counter()
        print("\nTime spent: ", time_new-time_now)
