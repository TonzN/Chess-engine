...
zobrist_keys = {
    piece_type: [[rand.getrandbits(64) for _ in range(64)] for _ in range(2)]  # 64 squares, 2 colors
...
def run(last_move, board):
  ...
  #  time.sleep(0.25)
    ...
    if game_vars["in_opening_book"]:
        ...
        search_data = negamax(board, game_vars["start_depth"], float('-inf'), float('inf'))
