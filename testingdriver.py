...
#test vars
use_engine = True
load_white_pieces = True
load_black_pieces = True
play_turns = True
...
game.loadPawns(load_white_pieces, load_black_pieces)
game.loadPieces(load_white_pieces, load_black_pieces)
engine.game_vars["debug_move"] = True
...
while run:
    ...
    if not player == board.turn and use_engine == True:
    ...
    if window.leftclick() == True: #Select
        if selected_piece: 
            ...
            if is_legal_move == True or play_turns == False: 
    ...
    if selected_promotion:
        ...
        cl.Piece(team, selected_promotion[1].piece_type, game.last_move, game.Positions[game.last_move], screen)
